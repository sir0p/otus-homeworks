...
control 'terraform' do
  ...
  describe command('cd terraform/prod && tflint --var-file=terraform.tfvars.example') do
    its('stdout') { should match "" }
    its('stderr') { should eq '' }
    its('exit_status') { should eq 0 }
  end

  describe command('cd terraform/stage && tflint --var-file=terraform.tfvars.example') do
    its('stdout') { should match "" }
    its('stderr') { should eq '' }
    its('exit_status') { should eq 0 }
  end
